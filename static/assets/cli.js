...
(function () {
  ...
  function sendCommand(st, line, output, promptEl, input) {
    ...
    if (typeof window.ProuterdWS === "undefined" ||
        typeof window.ProuterdWS.runCommand !== "function") {
      appendLine(output, "(WebSocket client not loaded)", "cli__line cli__line--error");
      return;
    }

    input.disabled = true;

    ProuterdWS.runCommand(trimmed, st.sessionId, {
